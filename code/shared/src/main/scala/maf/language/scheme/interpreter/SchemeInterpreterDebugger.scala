 ...
 */
class SchemeInterpreterDebugger[A <: SimpleModFAnalysis](
    ...
    with ConcreteSchemePrimitivesDebugger[A]():
...
object SchemeInterpreterDebugger:
    ...
    def evalPredicate[A <: SimpleModFAnalysis](pgm: SchemeExp, stateKeeper: StateKeeper[A]): Boolean =
        val interpreter = new SchemeInterpreterDebugger(stateKeeper, (id, v) => ())
        val res = interpreter.run(pgm, Timeout.start(timeout))
        if res == ConcreteValues.Value.Bool(true) then true
        else if res == ConcreteValues.Value.Bool(false) then false
        else true
...
/*def main(args: Array[String]): Unit =
    if args.length == 1 then
      ...
      val interpreter = new SchemeInterpreterDebugger((id, v) => ())
      val res = interpreter.run(pgm, Timeout.start(timeout))
